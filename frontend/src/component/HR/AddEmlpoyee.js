...
function EmployeeForm() {
    ...
    const [NICValidity, setNICValidity] = useState(false); // Added state to track NIC validity
    ...
    function sendData(e) {
        ...
        if (!NICValidity) {
            ...
            setNICError(""); // Clear the error message if NIC is valid
        }
        ...
        if (Gender !== "Male" && Gender !== "Female" && Gender !== "Not Disclosed") {
            ...
            setGenderError(""); // Clear the error message if gender is valid
        }
    ...
    // Function to handle NIC validation in real-time
